    ...
    var requestHandler = {
      'login-request': function() {
        if (self._authnRequestBinding === 'HTTP-POST') {
          ...
          this._saml.getAuthorizeUrl(req, redirectIfSuccess, options.additionalParams);
        }
      }.bind(self),
...
Strategy.prototype.logoutCallback = function(req, callback) {
  ...
  this._saml.validatePostRequest(req.query, function(err, profile, loggedOut) {
    if (loggedOut) {
      req.logout();
      if (profile) {
        req.samlLogoutRequest = profile;
        return self._saml.getLogoutResponseUrl(req, callback);
      }
    }
  });
};
