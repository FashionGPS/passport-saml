...
SAML.prototype.getAuthorizeUrl = function (req, callback, additionalParams) {
  var self = this;
  self.generateAuthorizeRequest(req, self.options.passive, function(err, request){
    if (err)
      return callback(err);
    var operation = 'authorize';
    let addiParams = self.getAdditionalParams(req, operation);
    addiParams = Object.assign(addiParams, additionalParams);
    self.requestToUrl(request, null, operation, addiParams, callback);
  });
};
...
SAML.prototype.validatePostRequest = function (container, callback) {
  var self = this;
  var xml = zlib.inflateRawSync(new Buffer(container.SAMLRequest, 'base64')).toString('utf8');
  ...
  parser.parseString(xml, function (err, doc) {
    ...
    // Our SLO is not signed, so do not verify any signature
